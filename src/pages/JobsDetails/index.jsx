...
const JobsDetails = () => {
  ...
  useEffect(() => {
    if (
      job &&
      savedJobs.some(
        (savedJob) => savedJob.jobId && savedJob.jobId._id === job._id
      )
    ) {
      setIsFav(true);
    } else {
  ...
  const handleFavIcon = (jobId) => {
    const savedJob = savedJobs.find(
      (savedJob) => savedJob.jobId && savedJob.jobId._id === jobId
    );
    ...
    if (job?.additionalJobForm) {
      ...
      try {
        ...
        const res = await dispatch(
          fetchAppliedJobsByJobSeeker({ userId })
        ).unwrap();
        res, "appliedJobs";
        setIsApplied(true);
  ...
  return (
    <>
      <section>
        <div className={`${styles.headBage}`}></div>
        ...
        <div className={styles.container}>
            ...
            <div className="col-lg-4 col-md-4 col-sm-12 p-0 col-12">
              <div className={styles.btns}>
                ...
                {isApplied ? (
                  ...
                  >
                    {" "}
                    {job?.additionalJobForm ? " Apply Now" : "Easy Apply"}
                  </button>
                )}
              </div>
            </div>
          ...
          {job.status !== "Open" ? (
            <div className="bg-info-subtle p-3 rounded-2 w-75 border-1 border border-info text-secondary">
              This position is currently closed. We are no longer accepting
          ...
          <div className="row mt-5 ">
            <div
              className={`${styles.description} col-lg-8 col-md-7 col-sm-12 mx-auto p-4`}
            >
              <div>
            ...
            <div className="col-lg-3 col-md-4 col-sm-12  mx-auto ">
              <div className={`${styles.sideBar} p-4`}>
                ...
                <ul className={`p-0 `}>
                  <li>
                    <JobSeekerProfileItem
                      width="100%"
                      backgroundColor={"var(--border02)"}
                      content={`${job?.jobLocation?.State}, ${job?.jobLocation?.government}`}
                      icon={UilMapMarker}
                    />
                  </li>
                  <li>
                  ...
                  <li>
                    <JobSeekerProfileItem
                      width="100%"
                      content={`${job?.salary?.from} - ${job?.salary?.to}`}
                      backgroundColor={"var(--border03)"}
                      icon={UilDollarAlt}
                    />
                  </li>
              ...
              <div className={`position-relative ${styles.JobsSideBar} `}>
                <label className={`${styles.OtherJobs}  px-3`}>
                  Other Jobs
                </label>
                <div className={`${styles.othersJobsSideBar} p-1 `}>
                        ...
                        <div key={job._id} className={styles.content}>
                          ...
                          <div className={`${styles.text} `}>
                            <Link to={`/JobsDetails/${job._id}`}>
                              <h5 className={styles.jobTitleSideBar}>
                                {job.JobTitle}
                              </h5>
                            </Link>
                            <p className="my-2">
                  ...
                  <button
                    ...
                    className="btn btn-success my-3 mx-auto w-50 d-block"
                  >
                    Show More
                  </button>
                </div>
      ...
      <PopUp
        ...
        body={
          <>
            <div className="w-100 ">
              <img src={congrats} alt="" width="80%" height="80%" />
            </div>
            ...
            <button
              className="d-block m-auto btn btn-success"
              onClick={handleClose}
            >
              Ok
            </button>
